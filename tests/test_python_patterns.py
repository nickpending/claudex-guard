...
"""
        ...
        try:
            ...
            # Filter for just type hint violations
            type_hint_violations = [
                v for v in violations if v.violation_type == "missing_type_hints"
            ]
            assert len(type_hint_violations) == 1
            assert "function_without_type_hint" in type_hint_violations[0].message
            assert type_hint_violations[0].function_name == "function_without_type_hint"
            assert type_hint_violations[0].ast_node is not None
        finally:
            file_path.unlink()
...
"""
    ...
    def test_analyze_patterns_antipatterns(self):
        ...
        try:
            ...
            # Check for key antipatterns
            mutable_default_found = any(
                r"def\s+\w+\([^)]*=\s*\[\]" in pattern for pattern in violation_types
            )
            bare_except_found = any(
                r"except\s*:" in pattern for pattern in violation_types
            )
...
"""
        ...
        try:
            ...
            pathlib_violations = [
                v for v in violations if v.violation_type == "missing_pathlib"
            ]
...
"""
        ...
        try:
            ...
            formatting_violations = [
                v for v in violations if v.violation_type == "old_formatting"
            ]
            assert len(formatting_violations) == 1
            assert (
                "old_percent"
                in formatting_violations[0].language_context["formatting_style"]
            )
        finally:
            file_path.unlink()
...
"""
        ...
        try:
            ...
            violations = self.patterns.analyze_development_patterns(
                code, lines, file_path
            )

            logging_violations = [
                v for v in violations if v.violation_type == "error_handling"
            ]
            assert len(logging_violations) == 1
            assert (
                "exception_without_logging"
                in logging_violations[0].language_context["pattern"]
            )
        finally:
            file_path.unlink()
...
"""
        ...
        try:
            ...
            composition_violations = [
                v for v in violations if v.violation_type == "composition_violation"
            ]
